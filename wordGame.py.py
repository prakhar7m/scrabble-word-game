...
def get_word_score(word, n):
    ...
    second_component = second_component if second_component > 1 else 1
...
def update_hand(hand, word):
    # convert word to a dict
    # get the new hand from difference of word_dict and hand
...
def is_valid_word(word, hand, word_list):
    word = re.sub('[^a-zA-Z*]+', '', word).lower()
    available_letters = Counter(hand)
    if word.find("*") == 1:
        for letter in 'aeiou':
            word_new = word.replace("*", letter)
        ...
        if word not in word_list or Counter(word) - available_letters:
            return False
        else:
            return True
...
def calculate_handlen(hand):
    ...
    cards = sum(hand.values())                               # we can get an empty hand
                    # and count the cards!
    return cards
    """ 
...
def play_hand(hand, word_list):
    cards_left = calculate_handlen(hand)
    total_score = 0
    ...
    if cards_left == 0:
        print('Run out of letters.')  # hand exit status

    return total_score


play_hand(deal_hand(n), word_list)
